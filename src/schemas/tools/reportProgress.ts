...
import { z } from 'zod';
import { type ObjectiveStatus, type AgentStatus, agentStatusSchema } from '../index.js';
...
export const ProgressReportMetadataSchema = z.object({
  objectiveId: z.string().optional().describe("Associated objective ID for linking progress to specific objectives"),
  ...
  progressPercentage: z.number().min(0).max(100).optional().describe("Progress percentage (0-100) for objective completion tracking"),
  results: z.record(z.string(), z.any()).optional().describe("Objective results or metadata as key-value pairs containing objective outcomes"),
...
export const ProgressReportInternalSchema = z.object({
  ...
  progressType: z.enum(['status', 'objective', 'milestone', 'error', 'completion']).describe("Type of progress being reported: 'status' for agent status updates, 'objective' for objective progress, 'milestone' for achievements, 'error' for failures, 'completion' for objective completion"),
  message: z.string().describe("Human-readable progress message describing what the agent is doing or has accomplished"),
  metadata: ProgressReportMetadataSchema.optional().describe("Additional progress metadata containing objective details, status changes, results, and context")
});
...
export const ProgressReportResponseDataSchema = z.object({
  ...
  progress_type: z.string().describe("Type of progress reported (status, objective, milestone, error, completion)"),
  ...
  objective_id: z.string().optional().describe("Associated objective ID if the progress was related to a specific objective")
});
